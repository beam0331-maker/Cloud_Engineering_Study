...
public class TodoController {
    ...
    public ApiResponse<List<TodoDTO>> todos(){
        return ApiResponse.ok("Todo list success",service.findAll());
    }
    ...
    public ApiResponse<TodoDTO> findById(@PathVariable int id){
        TodoDTO dto  = service.findById(id);
        if(dto == null){
            return ApiResponse.fail(id + " not find",service.findById(id));
        } else{
            return ApiResponse.ok(id + " find success",service.findById(id));
        }
    ...
    *   요청방법
    ...
    @PostMapping("/todos")
    public ApiResponse<TodoDTO> creatTodo(@RequestBody TodoDTO todoDTO){

        int n = service.save(todoDTO);

        return ApiResponse.ok("save sueccess",todoDTO) ;
    }
    ...
    public ApiResponse<TodoDTO> updateTodo(@PathVariable int id,
        ...
        return ApiResponse.ok("update success",todoDTO);
    }
    ...
    public ApiResponse<Void> deleteTodo(@PathVariable int id){
        ...
        return ApiResponse.ok("delete success",null);
    }
