...
public class StudentMain {
		...
		while (true) {
			...
			System.out.print("""
					...
					***********************************************
					...
					1. 전체 학생 목록
					2. 학생 이름 검색
					3. 학생 입학년도 범위 검색 (예> 2000년부터 2003년까지)
					4. 학생 학번으로 다중 검색 (쉼표 구분자)
					5. 학생 휴학 일괄 수정
					6. 학과 정원 일괄수정
					7. 학생 학점 검색
					***********************************************
	...
	static void stuList() {
		...
		List<StudentDTO> stuList = service.list();
		...
		for (int i = 0; i < stuList.size(); i++) {
			...
			System.out.printf("%s %s %s %-16s %-13s %-1c\n", stu.getStuNo(), stu.getStuName(), stu.getStuSsn(),
					stu.getStuAddress(), stu.getDate(), stu.getAbsYn());
		}
		System.out.println("총 학생수 : " + stuList.size() + " 명");
	}// stuList ()

	static void studNameSerch(String findName) {

		StudentService service = new StudentServiceImpl();
		service.setDao(new StudentDAO());
		List<StudentDTO> stuList = service.nameSearch(findName);
		System.out.print("""
				================================================================================
				학번	이름	주민번호		주소		입학년도	휴학여부
				--------------------------------------------------------------------------------
